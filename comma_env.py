...
class BrickBreakerCommaEnv(gym.Env):
    ...
    def _get_ascii(self):
        ...
        # Add a comma before the paddle, if there is room (i.e. not overriding the left border)
        if self.paddle_x > 1:
            grid[self.paddle_y][self.paddle_x - 1] = ","
        ...
        # Add a comma before the ball, if there is room
        if bx > 1 and 0 <= by < self.WIN_HEIGHT:
            grid[by][bx - 1] = ","
