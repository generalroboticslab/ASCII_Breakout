...
from envs.breakout_comma_env import BrickBreakerCommaEnv
from envs.breakout_no_comma_env import BrickBreakerEnv
from envs.asteroids_no_comma_env import AsciiAsteroids
from envs.galaxian_no_comma_env import AsciiGalaxian
...
def extract_action(output_decode: str) -> int:
    pattern = r"<action>\s*([01234])\s*</action>"
...
if __name__ == "__main__":
    ...
    api_value = input("Do you want to run OpenAI GPT-4o (1),  OpenAI GPT-4o-mini (2), OpenAI o1-mini (3), OpenAI o3-mini (4), Llama 3.2 3B (5)?, Random (6), or ALL (7)")
        ...
        for m in range(len(models)):
            for p in prompts:
                for test in range(tests_per_model):
                    ...
                    if p == 'small':
                        prompt = configurator.build_prompt(
                            game=e, 
                            include_universal_steps=[
                            ]
                        )
                    ...
                    if e=="breakout":
                        ...
                        frames.append(ascii_to_image(obs, num_cols=62, num_rows=32))
                    elif e=="galaxian":
                        frames.append(ascii_to_image(obs, num_cols=42, num_rows=22))
                    ...
                    responses_filename = f"./all_responses_{e}_{models[m]}_prompt_{p}_{comma_str}_{critic_str}_{timestamp}.txt"
                    with open(responses_filename, "a") as file:
                        file.write('')
                    file.close()
                    # Run for 1000 actions
                    # Replicate Atari-GPT
                    for i in tqdm(range(1000), desc="Testing"):
                        action = None
                                    ...
                                    for _ in range(3):
                                        ...
                                        messages.append({
                                                ...
                                                "content": 
                                                        ...
                                                        Your action output should be an integer within the action tags like this <action> 'X' </action>"""
                                                    }
                                                )
                        ...
                        if e=="breakout":
                            ...
                            img = ascii_to_image(obs, num_cols=62, num_rows=32)
                        elif e=="galaxian":
                            img = ascii_to_image(obs, num_cols=42, num_rows=22)
                        frames.append(img)
                    ...
                    video_filename = f"{e}_{models[m]}_prompt_{p}_{comma_str}_{critic_str}_{timestamp}.mp4"
                    ...
                    csv_filename = f"./actions_rewards__{e}_{models[m]}_prompt_{p}_{comma_str}_{critic_str}_{timestamp}.csv"
